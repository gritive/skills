...
**본문을 조회하지 않는다** — 참조 문자열만 넘기고 조회는 리뷰어가 한다(Phase 3).
...
이 세션이 이슈를 읽고 골라 넘기지 않는 이유: 여기는 Phase 2에서 이미 diff와 파일 목록을 봤다.
그 상태로 이슈에서 무엇이 요구인지 고르면 **코드가 다루는 부분만 골라 넘기게 되고**, "요구했는데
없는 것"이 판정 전에 걸러진다. 기준을 고르는 주체와 판정하는 주체를 같게 둔다.
`빌더의 Coverage Plan·Audit 표를 넘기지 않는 것`과 같은 이유다(그 문서의 "기준은 둘이고 층이
다르다" 절). **빌더의 Coverage Plan·Audit 표는 여전히 넘기지 않는다.**
...
참조를 제거함")를 명시한 항목은 `files` 밖이어도 **유지한다.** 근거가 없으면 제거한다.
