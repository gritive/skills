...
| 리뷰어            | CLAUDE.md에서 읽는 정보                              |
| ----------------- | ---------------------------------------------------- |
| backend           | 아키텍처 원칙·계층 구조, 코딩·네이밍 컨벤션, 기술 스택(언어별 분석 도구)·특수 진입점, 성능 규칙·DB 인덱스 요구사항 |
| security          | 보안 규칙, 인증/인가 정책, 데이터 보호 요구사항      |
| frontend          | 프론트엔드 프레임워크, 컴포넌트 규칙, 상태 관리 패턴 |
