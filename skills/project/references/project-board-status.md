...
2. 후보와 각 상위 에픽을 각각 확인한다. `Todo`·`In Progress`만 build 후보가 될 수 있다는 `build.md`의 규칙은 그대로 적용한다. 카드·열을 찾을 수 없으면 상태를 추측하지 말고 중단한다.
3. 이미 `In Progress`인 항목은 이 UI 확인만으로 계속 진행할 수 있다. 보드 연결 또는 `Status` 변경이 필요한 항목은 카드의 UI 상태 편집으로 처리하고, 변경 뒤 같은 카드가 `In Progress` 열에 있는지 다시 확인한다.
