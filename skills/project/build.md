...
- 라벨이 비어 있는 저장소가 흔하다(`labels: []`). 그럴 땐 **제목 접두어·본문 내용**으로 분류한다.
  ...
  **에픽은 제외 클래스가 아니라 실행 그래프의 루트다.** `[Epic]` 접두어, native sub-issue, 본문의
  ...
  4. **에픽 자체나 남은 하위 작업이 선행 미완·게이트·credential·question·`build-blocked`에만
     ...
     이런 에픽이 남으면 `excluded_only`가 아니라 `blocked`로 에픽 번호·막힌 대상·해제 조건을
     보고한다. 이것은 구현·리뷰·CI 실패 같은 실행 중단이 아니라 외부 해제를 기다리는 제품 범위다.
     전역 build는 그 에픽의 서브트리 잠금을 풀고 다음 독립 후보를 계속 고른다. scope build는 범위
     밖 후보로 나가지 않으므로 즉시 `blocked`를 보고한다. 어느 경우에도 미완료 제품 범위를
     백로그 소진이나 수렴으로 세지 않는다.
  ...
  **전역 build의 보류 목록** — 이 호출에서 `blocked`가 된 에픽 root와 scope closure를 보류 목록에 넣고,
  ...
  후보를 처리할 수 있다. 이 목록은 보드 Status를 바꾸지 않으며, 완료 보고에서 모든 에픽의 막힌 leaf와
  해제 조건을 함께 낸다.
...
클래스임이 열거로 확인될 때 종료한다.** 후자에 미완료 에픽 자체가 제외됐거나 에픽이 그 제외 이슈를
leaf로 소유하면 전역 build는 다른 독립 후보를 계속 처리하고, 더 이상 후보가 없을 때 모든 보류 에픽의
번호·막힌 대상·해제 조건과 함께 `blocked`로 보고한다. scope build도 같은 내용을 `blocked`로 보고한다. 미완료
에픽이 없을 때만 `excluded_only`로 완료 보고를 낸다(완료 보고 절).
...
### 3.25. 착수 직전 refresh·보드 연결·진행 상태 반영
...
**선정 결과는 예약이 아니다.** 위 fetch 뒤, 보드 연결·상태 변경·브랜치 생성·subagent dispatch 전에
선택한 이슈를 다시 조회한다. 1~3단계에서 받은 응답을 재사용하지 않는다.

- 3단계의 REST 조회로 최신 이슈 상태·본문·라벨·댓글을 읽고, 2단계의 선행 관계와
  [references/project-board-status.md](references/project-board-status.md)의 단건 보드 Status도 새로 읽는다.
  최신 내용에 0.5·2·3단계의 신뢰 경계와 후보 자격을 다시 적용한다. scope가 있으면 현재 scope 소속도 재확인한다.
- 닫혔거나 제외 조건에 해당하면 선택을 취소하고 사유를 남긴 뒤 1단계로 돌아간다.
  단, 아래 `recovery_required` 처리에서 진입했다면 원 이슈 번호·브랜치·stash 참조를 보존해
  `stopped`로 보고한다. 원 작업 복귀를 건너뛰고 일반 백로그를 재선정하지 않는다.
  특히 다른 실행 세션이 `In Progress`로 바꿨으면 그 상태를 덮어쓰거나 구현을 시작하지 않는다.
  상위 에픽의 상태로 leaf를 제외하지 않는 2단계 규칙은 그대로 적용한다.
- 묶음이면 **모든 구성원을 다시 검사한 뒤에만** 아래 변경을 시작한다. 하나라도 부적합하면
  묶음 전체의 선택을 취소하고 1단계에서 다시 선정한다. 이후 구현에는 refresh한 본문·댓글을 전달한다.
- 조회 실패나 불완전한 응답으로 현재 자격을 확인할 수 없으면 `stopped`로 멈추고 보고한다.
  refresh는 원자적 잠금이 아니므로 동시 착수를 완전히 막는 소유권 증명으로 취급하지 않는다.
...
이 세션은 0~3.25단계(선정·보드 상태 반영)와 9~11단계(PR·머지·다음 이슈)만 한다.
...
| 현재 이슈 보류 | `issue_blocked` + 사유·실제 출력(해당 시)·작업 브랜치명·완료/미완료 범위 |
| 전체 중단 | `stopped` + 어느 중단 조건인지와 근거                                                                                                                                                                                                                              |
...
클래스도 아니기 때문이다. 그래서
넘어가기 전에 **그 이슈에 실패 사유·실제 검증 출력(해당 시)·작업 브랜치명을 코멘트로 남기고
`build-blocked` 라벨을 붙인다.** 2단계의 "항상 제외"가 이 라벨을 거르므로 루프가 같은 이슈를 영원히
돌지 않고 다음 독립 이슈로 간다. 코멘트나 라벨을 못 남겼거나 다음 브랜치로 전환할 깨끗한 워킹 트리를
...
**`references/review-gate.md`를 읽고 그대로 수행한다** — PR 전 최신 base 통합, 리뷰 실행,
`review_status`·`gate_status`, 발견 처리, 패스 예산, PR 본문이 전부 그 문서에 있다. 이 단계의 산출은 그 문서의 7번 표이고,
...
- **제외(선정·착수 직전 refresh, 0~3.25단계)**: 아직 워킹 트리를 건드리지 않았다. 부적합 이슈는 **건너뛰고 다음 이슈로 간다.**
...
- 2패스를 살아남은 발견은 중단이 아니다 — `unresolved`로 등록하고 진행한다. 리뷰 수정 후 재검증 명령
  자체가 실패한 경우도 `build-blocked`로 보류하고 다음 독립 이슈로 간다(9.5-6)
- **CI 실패** (10단계) — 미룰 수 없다
...
기계 판정 계약은 `drained | excluded_only | blocked | stopped` 네 값만 사용한다. scope를 받았으면 별도 줄에
`scope: issue-closure:#N`도 명시한다. 네 outcome은 보고한 scope 기준이며, 전역 실행은 기존처럼 전체 백로그 기준이다.

**마지막 이슈를 끝내거나 멈출 때는 아래 항목에 더해 `outcome`을 한 줄로 명시한다** —
`drained`(빌드가능 백로그 소진·에픽 정합성 완료) / `excluded_only`(미완료 에픽 없이 독립 제외 클래스만
남음) / `blocked`(독립 후보를 모두 소진했지만 외부 해제를 기다리는 미완료 에픽이 남음) /
`stopped`(구현·리뷰·CI·권한 등 실행 중단 조건).
**`blocked`면 모든 보류 에픽의 번호·막힌 leaf·해제 조건을, `stopped`면 어느 실행 중단 조건인지와 근거를 원문
전달한다.** `loop`이 이 한 줄로 다음 Phase·독립 작업 계속·루프 중단을 가르므로, 없으면 판정하지
못하고 멈춘다(`loop.md` Phase A/B/C).
