...
> web 인증은 스킬이 자동 입력하지 않는다(안전 규칙). 위 계정 정보는 사용자가 직접 로그인할 때
> 참고용이며, 이미 로그인된 브라우저 세션이 있으면 그대로 재사용한다.

### 페르소나
| ID  | 이름        | 역할        | 기술수준·인내심 | 업무 목표                | 시나리오             | 핵심 검증      |
| --- | ----------- | ----------- | --------------- | ------------------------ | -------------------- | -------------- |
| P-1 | 신규 사용자 | 첫 방문자   | 비기술·낮음     | 회원가입 후 첫 작업 완료 | 가입→설정→첫 사용    | 온보딩 흐름    |
| P-2 | 파워유저    | 일상 사용자 | 보통·높음       | 일일 루틴 효율적 수행    | 로그인→주요작업→완료 | 반복 작업 효율 |
| P-3 | 관리자      | 팀 관리     | 보통·낮음       | 현황 파악 및 의사결정    | 대시보드→확인→조치   | 정보 가시성    |

> `기술수준·인내심`은 스킬이 시뮬레이션 중 능력을 낮추는(capability throttling) 기준이다.
> 명시하지 않으면 유능한 모델이 실제 유저라면 막혔을 마찰을 뚫고 지나가 과소평가한다.
